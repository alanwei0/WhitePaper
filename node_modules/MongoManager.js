...
MongoManager.prototype.joinRoom = function(data , callback){
    ...
    self.db.collection('rooms').findOne({'_id': roomId , 'password': password} , function(err, doc){
        ...
        if(doc){
            console.log(doc);
            self.db.collection('canvas').findOne({'_id': doc._id} , function(err,doc){
                if(err){
                    console.log(err);
                    return callback([0,err]);
                }
                console.log(doc);
                if(doc){
                    return callback([1 , [roomId , user , doc.img]]);
                }else{
                    return callback([1 , [roomId , user , '']]);
                }
            });
        }else{
            return callback([0,'not exist']);
        }
    });
};
...
MongoManager.prototype.saveCanvas = function(data , callback){
    ...
    self.roomExist(roomId , function(data){
        console.log(data);
        if(!data[0]) return callback(data);
        self.db.collection('canvas').save({'_id': roomId , 'img': body});
    });  
};
...
MongoManager.prototype.checkRoomId = function(roomId){
    ...
    if(!roomId.match(/^\w+$/)) return [0,'roomId is illegal'];
    return [1];
};
